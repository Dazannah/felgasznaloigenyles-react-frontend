    ...
    return (
      <div key={request.email + "DivKey"} id={index+ request.email + "Div"} className="request">
        ...
        <div key={request.index + "contentKey"} id={index + "content"} className="collapsibleContent ">
          <DistributionListFields showButtons={true} request={request} generateInputFieldsNow={generateInputFieldsNow} setGenerateInputFieldsNow={setGenerateInputFieldsNow} inputFieldNumber={request.emailsCount} setGetRequests={setGetRequests}/>
        </div>
      </div>
    )
  }
